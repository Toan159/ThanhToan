<section class="footer " style="background-image: url('@asset('./images/default/Group 2692.png')'); height:500px">
  <div class="container">
    <div>
      <div class="bg-orange-100 "><img src="@asset('./images/default/footer.png')"></div>
      <div class="">
        <span class="icomoon icon-youtube"></span>
        <span class="icomoon icon-instagram"></span>
        <span class="icomoon icon-twitter"></span>
        <span class="icomoon icon-facebook"></span>
        <span class="icomoon icon-linkedin2"></span>
      </div>
      
    </div>
  </div>
</section>
{{-- <footer id="footer" class="module footer h-[350px]" style="background-image: url('@asset('./images/default/Group 2692.png')'); ">
  <div class="container footer-top ">
    <div class="row">
     
      <ul class="list-none col p-0 w-1/2 lg:w-2/3">
        <li>
          <!-- <span class="icomoon icon-chevron-right">alo alo</span> -->
          <!-- <img class="w-full" src="@asset('./images/default/Group 2692.png')"> -->
        </li>
        <!-- {!! App::getFooterNav() !!} -->
      </ul>
      <div class="footer-item-col col w-1/2 lg:w-1/3">
        {!! App::getFooterAddress() !!}
      </div>
    </div>
  </div>
  <div class="container text-center footer-bottom">
    <div>
      <div>
        <img src="@asset('./images/default/footer.png')">
      </div>
      <div></div>
      <div></div>
      <div></div>
    </div>
    <div class="footer-copyright last-mb-none p-3 h-[350px]">
      {!! App::getCopyRight() !!}
       <img src="@asset('./images/default/Group 2692.png')">
    </div>
  </div>
</footer>

endblock 
<noscript>
  <div id="mod-noscript" class="mod-noscript bg-black text-white fixed inset-0 z-200">
    <div class="table w-full h-full">
      <div class="table-cell align-middle text-center">
        <div class="container last-mb-none">
          <h3>To use web better, please enable Javascript.</h3>
        </div>
      </div>
    </div>
  </div>
</noscript> --}}